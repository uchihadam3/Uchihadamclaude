...
export const numeroFlutuante = (
  ...
  const desvio = (Math.random() - 0.5) * 34;
  cena.tweens.add({
    targets: rotulo,
    y: y - 42,
    x: x + desvio,
...
export const estilhacos = (
  ...
  for (let i = 0; i < quantidade; i += 1) {
    ...
    const forca = 40 + Math.random() * 90;
    const lasca = cena.add.rectangle(x, y, 3, 3, cor).setDepth(870);
    cena.tweens.add({
      targets: lasca,
...
export const anelDeRuptura = (cena: Phaser.Scene, x: number, y: number): void => {
  const anel = cena.add.circle(x, y, 10).setDepth(885).setStrokeStyle(3, 0x8ad6ff, 1).setFillStyle();
  cena.tweens.add({
    targets: anel,
    scale: { from: 0.5, to: 3.2 },
    alpha: { from: 1, to: 0 },
    duration: 420,
    ease: 'Cubic.easeOut',
    onComplete: () => {
...
export const arcoDeGolpe = (
  ...
  const grafico = cena.add.graphics().setDepth(875);
  const sentido = paraDireita ? 1 : -1;
  grafico.lineStyle(4, Number.parseInt(ajustar(cor, 0.4).slice(1), 16), 0.95);
  grafico.beginPath();
  grafico.arc(x, y, 34, -Math.PI * 0.42 * sentido, Math.PI * 0.42 * sentido, !paraDireita);
  grafico.strokePath();
  cena.tweens.add({
    targets: grafico,
    alpha: { from: 1, to: 0 },
