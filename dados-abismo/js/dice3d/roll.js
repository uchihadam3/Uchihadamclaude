...
export function rolarPara(tipo, faceAlvo, semente0, mesa, maxTentativas=180, zona=null){
  let melhor=null;
  for(let k=0;k<maxTentativas;k++){
    const semente = (semente0 + k*7919)|0;
    const d = new Dado(tipo, 0.5);
    const trilha = simular(d, mulberry(semente), mesa, 900, 1/120, zona);
    if(!d.dormindo || trilha.length > 260) continue;   // queda longa demais = descarta
    const f = faceParaCima(tipo, d.q);
    if(f === faceAlvo) return { trilha, faceIdx:f, semente, tentativas:k+1, exato:true };
    if(!melhor) melhor = { trilha, faceIdx:f, semente, tentativas:k+1, exato:false };
  }
