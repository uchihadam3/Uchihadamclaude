...
const D: Record<string, string> = {
  ...
  'Sem senha a sala fica': 'Without a password the room is', 'livre': 'open',
  'no salão; com senha 🔒 só entra quem souber. As vagas ficam': 'in the lounge; with a password 🔒 only those who know it get in. Seats stay',
  'abertas': 'open', '— IA só entra se você adicionar.': '— AI only joins if you add it.',
  'vagas na sala': 'seats in the room', '➕🤖 Adicionar IA': '➕🤖 Add AI',
  'toque no ✕ da IA pra tirar': 'tap the ✕ on an AI to remove it',
  'vaga livre é pra gente — IA só se você quiser': 'open seats are for people — AI only if you want',
  'vaga livre — esperando alguém entrar…': 'open seat — waiting for someone to join…',
  'Sala vazia — espere alguém entrar ou adicione uma 🤖 IA': 'Empty room — wait for someone or add an 🤖 AI',
  'Dupla precisa da sala completa — adicione 🤖 IA ou espere gente': 'Doubles needs a full room — add 🤖 AI or wait for people',
...
const RULES: Rule[] = [
  ...
  [/^👤 (\d+) · 🤖 (\d+) · vagas livres: (\d+)$/, m => `👤 ${m[1]} · 🤖 ${m[2]} · open seats: ${m[3]}`],
