...
- Fundo **totalmente transparente** (PNG com alpha). Se a IA insistir, um verde-limão chapado ou o xadrez também dão pra recortar.
- A **base do objeto encostada na borda de baixo**, centralizada (é o "pé" que fica no chão).
- Vista **frontal reta**, luz difusa de dia encoberto, **sem sombra no chão** e **sem névoa** pintada (a névoa é do motor).
- Sem texto / marca d'água.
- Manda **uma imagem por vez com o nome** (ex.: "esse é o Aglomerado 2") que eu recorto, integro e te mostro no jogo.

---

## 01 · Árvores individuais

Retrato alto (~1024×1600). Viram planos cruzados no interior da mata.

### ✅ Árvore 1 — abeto alto e estreito (pronto)

```
Hand-painted stylized-realism game asset, cohesive with a grim medieval fantasy village. A single tall narrow fir tree, full trunk visible with the base of the trunk touching the very bottom-center edge of the frame, tree fills the vertical space. Coniferous, dense layered branches, tall pointed silhouette. Cool desaturated palette (deep greens, blue-grey shadows), soft overcast lighting, light frost/snow dusting on the tips. Flat front-facing orthographic view, no perspective distortion, even diffuse lighting so it reads from any angle, no ground cast-shadow. Clean cutout on a fully transparent background, subject centered, crisp edges, no atmospheric haze or fog baked in, no text, no watermark. Tall portrait canvas ~1024x1600.
```

### ✅ Árvore 2 — pinheiro largo e cheio (pronto)

```
Hand-painted stylized-realism game asset, cohesive with a grim medieval fantasy village. A single broad full pine tree, lush and wide with heavy layered branches, full trunk visible with the base of the trunk touching the very bottom-center edge of the frame, tree fills the vertical space. Cool desaturated palette (deep greens, blue-grey shadows), soft overcast lighting, light frost/snow dusting on the tips. Flat front-facing orthographic view, no perspective distortion, even diffuse lighting so it reads from any angle, no ground cast-shadow. Clean cutout on a fully transparent background, subject centered, crisp edges, no atmospheric haze or fog baked in, no text, no watermark. Tall portrait canvas ~1024x1600.
```

### ✅ Árvore 3 — árvore morta e torta (pronto)
...
## 02 · Aglomerados de mata

Paisagem larga (~2048×1280). Muralha de árvores pro fundo — **bordas esquerda/direita tileáveis** (as árvores continuam pra fora do quadro).
...
```
Hand-painted stylized-realism game asset, cohesive with a grim medieval fantasy village. A dense cluster of 4 to 6 overlapping fir trees forming a low even treeline wall, seen straight-on, trunk bases aligned along the bottom edge of the frame, mostly similar heights with soft undergrowth and small bushes at the base. The left and right edges are tileable so copies placed side by side blend into one endless treeline. Cool desaturated palette (deep greens, blue-grey shadows), soft overcast lighting, faint frost on the tips. Flat front-facing orthographic view, no perspective distortion, even diffuse lighting, no ground cast-shadow. Clean cutout on a fully transparent background, crisp edges, no atmospheric haze or fog baked in, no text, no watermark. Wide landscape canvas ~2048x1280.
```
...
Representa o vilarejo atual: casas **simples de madeira com telhado de palha em fileira** (NÃO empilhadas uma sobre a outra), a **entrada da cidade** (portão de madeira), e uma **montanha rochosa maciça atrás** — **sem buraco/masmorra** (a dungeon fica no subsolo, não se vê). As **árvores eu adiciono no motor**, então não precisa. O resto some na neblina.
...
### ✅ Sprite 2 — espada em golpe / corte (pronto) — já no jogo; o motor troca pra ele no instante do corte
...
```
Hand-painted stylized-realism MOBILE game HUD frame for the bottom-right corner of the screen, cohesive with a grim medieval dark-fantasy game — SAME materials as the game's iron-and-gold buttons: aged dark pitted wrought iron, worn tarnished gold/brass trim, small rivets, faint arcane engraving. The MAIN body is a small UPRIGHT RECTANGULAR ornate plaque (a tall little panel, taller than wide) holding SIX empty round ability sockets in a STRICT 2x3 GRID — exactly TWO STRAIGHT VERTICAL COLUMNS, each column a stack of THREE sockets, one directly above another, aligned in perfectly even rows and columns like a 2-wide 3-tall number keypad. IMPORTANT: the sockets are NOT arranged in an arc, fan, curve, circle, ring, wheel, radial spray or crescent — they form a plain straight rectangular grid, the two columns perfectly vertical and parallel, the three rows perfectly horizontal. BELOW that rectangular plaque, at the very bottom-right corner, sits ONE LARGER empty round socket (the attack button); just to the LEFT of that large socket sits ONE medium empty round socket (the action button). Along the very BOTTOM edge, running to the left, a thin recessed horizontal CHANNEL (an experience-bar track), empty and dark with a subtle inner bevel. Every socket is a deep recessed dark metal cup framed by a beveled worn-gold rim with tiny rivets, completely EMPTY inside (no icons, no symbols, no runes). Gothic scrollwork ties the rectangular grid plaque, the two round buttons and the bottom XP rail into one forged piece. Compact, tight to the corner. Subtle worn metallic highlights, soft even diffuse lighting, no cast shadow, flat front orthographic straight-on view, no perspective. Clean cutout on a fully transparent background, crisp edges, no icons, no numbers, no text, no watermark. Wide landscape canvas ~1536x820, everything clustered in the lower-right, the 2x3 grid as two vertical columns, the thin XP rail along the bottom edge.
```

> **Se a IA insistir no leque/arco:** peça só a versão modular — **1 encaixe redondo
> vazio** (uso o `btn_base` que já existe) — e eu monto a **grade 2×3 por código** com
> a moldura. Assim garanto as duas colunas retas mesmo que a peça inteira não saia.
...
## 13 · HUD — Mapa e Relógio dia/noite

Duas peças novas do HUD. O **botão de inventário já foi movido pra ESQUERDA** (abaixo
da placa de vida/mana) pra liberar o **canto superior direito**, onde entra o **mapa**.
O **relógio** usa o ciclo de dia/noite que o jogo já tem (o sol acende de dia e vai
apagando, a lua acende de noite — eu faço esse *crossfade* por código).

> **Fundo:** magenta chapado (`#ff00ff`) pra recorte limpo (a moldura contra o magenta,
...
código dentro dela. Cantos bem definidos (uso 9-slice se precisar esticar).
...
como aro, ou você manda um aro dedicado depois).
...
### ✅ Peça E — Folha GENÉRICA de PASSIVAS (NO JOGO — todas as classes)
...
## 16 · Texturas de parede da masmorra (variações) — ✅ NO JOGO
...
## 17 · Baú 3D — faces em PNG (objeto 3D vestido com arte) — ✅ NO JOGO
...
## 21 · Ícones dos ATRIBUTOS (Força / Destreza / Inteligência) — 🟡 arte pendente

**Uso:** aparecem (a) no **card da skill** ao lado do dano (mostra com qual atributo ela
escala) e (b) na **aba Atributos** do personagem, ao lado de cada atributo. São 3 emblemas
**do mesmo conjunto** (mesmo enquadramento/tratamento), só mudando o símbolo e a cor.

**Regras dos 3 (conjunto coeso):**
- **Emblema** num pequeno **medalhão de pedra & ferro** escuro (grim medieval), pintado à
  mão, metal desgastado/gravado, com um **brilho colorido** por trás do símbolo.
- **Fundo 100% TRANSPARENTE** (alpha), recorte limpo, **legível mesmo pequeno** (vai ser
  exibido a ~24–40px), símbolo bem centralizado e cheio (sem margem grande).
...
> Ao soltar os 3 (`attr_forca.png`, `attr_destreza.png`, `attr_int.png`) em
> `grimhollow/src/assets/ui/`, eu ligo eles no **card da skill** (selo ao lado do dano,
> com a versão híbrida do Clérigo mostrando dois selos) e na **aba Atributos**. Enquanto
> não vierem, uso um símbolo genérico provisório (não trava nada).
...
lâmina d'água, raízes pendentes, e **cogumelos bioluminescentes** dando um brilho
**ciano/verde-frio** no breu. Paleta **fria** (teal/verde-azulado/petróleo), o oposto
da pedra quente do Ato I. Chega depois de derrotar o chefe do Ato I.
...
> **Ajuste (parede cinza LIMPA + teto novo):** o Ato II passou a usar a parede
...
**Inimigos do Ato II (billboards 2D, mesma regra do §24):** `...single creature
FACING THE VIEWER, full body, feet at the very bottom edge, hand-painted
stylized-realism, grim dark fantasy, dramatic even lighting, no cast shadow, FULLY
...
```
A COLOSSAL DROWNED LEVIATHAN boss, a towering bloated abomination risen from the flooded depths, TWICE the height and bulk of a man, dominating the whole frame — swollen waterlogged grey-green flesh split open to reveal a forest of glowing cyan fungal growths and spore-sacs, long barnacle-crusted arms ending in webbed claws, a gaping maw ringed with jagged teeth and dangling tendrils, several glowing pale eyes, cascading water and dripping slime, a cloud of luminous spores and cold mist swirling around it, a menacing wide stance radiating dread, feet at the very bottom edge. Cool teal-green bioluminescent palette. Hand-painted stylized-realism, grim dark fantasy, extremely detailed and threatening, cinematic. FULLY TRANSPARENT background (alpha channel), crisp cutout edges, no ground, no base, no text, no watermark. Tall portrait ~1280x1600.
```

> **Como integra:** você gera essas peças (uma por vez, com o nome), eu recorto e
> **ligo um tileset por ATO** (o Ato II carrega `tex_a2*` + este roster) e construo os
> 3 mapas novos (mesma engine de corredor). O checkpoint/portal que acabei de fazer já
> leva o jogador ao andar mais fundo — então, quando o Ato II existir, "Continuar" já o
> alcança e o chefe do Ato I abre a passagem pra cá.
